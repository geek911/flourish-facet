...
from .mother.flourish_consent_listboard import my_fun
...
class HomeView(EdcBaseViewMixin, NavbarViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        facet_appointment = self.facet_appointment_cls.objects.all().filter(
            appt_status='complete')
        flourish_consents = self.flourish_consent_cls.objects.all()

        eligible_subjects = my_fun(flourish_consents).count()
