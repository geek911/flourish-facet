...
from ...forms import MotherLocatorForm
from ...models import MotherLocator, FacetConsent


@admin.register(MotherLocator, site=flourish_facet_admin)
class MotherLocatorAdmin(ModelAdminMixin, admin.ModelAdmin):
    form = MotherLocatorForm

    def response_add(self, request, obj, **kwargs):
        response = self._redirector(obj)
        return response if response else super(MotherLocatorAdmin, self).response_add(request, obj)

    def response_change(self, request, obj):
        response = self._redirector(obj)
        return response if response else super(MotherLocatorAdmin, self).response_change(request, obj)

    def _redirector(self, obj):
        caregiver_locator = FacetConsent.objects.filter(
