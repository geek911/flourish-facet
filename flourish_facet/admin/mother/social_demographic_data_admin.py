...
class HouseHoldDetailsInlineAdmin(StackedInlineMixin, ModelAdminFormAutoNumberMixin, admin.StackedInline):
    ...
    def get_extra(self, request, obj=None, **kwargs):
        ...
        facet_visit = request.GET.get('facet_visit', '')

        twin_enrol = self.twin_enrolment(subject_identifier)

        child_sidx = self.onschedule_sid(obj, facet_visit_id=facet_visit)
    ...
    def onschedule_sid(self, obj, facet_visit_id=None):
        facet_visit = getattr(obj, 'facet_visit', None)
        if not facet_visit and facet_visit_id:
            facet_visit = FacetVisit.objects.get(pk=facet_visit_id)
    ...
    def subject_consents(self, subject_identifier=None):
        return FacetConsent.objects.filter(subject_identifier=subject_identifier)
...
class SocioDemographicDataAdmin(CrfModelAdminMixin, admin.ModelAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if len(inlines) > 0:
            ...
            child_sidx = household_instance.onschedule_sid(obj, facet_visit_id=facet_visit)
